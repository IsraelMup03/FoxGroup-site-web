...
export default function EquipeForm({ membre, onFermer, onEnregistre }) {
  ...
  return (
    <Modal titre={edition ? 'Modifier le membre' : 'Ajouter un membre'} onFermer={onFermer} large>
      <form onSubmit={envoyer} className="grid gap-5 md:grid-cols-[180px_1fr]">
          ...
          <label className="group relative block aspect-square cursor-pointer overflow-hidden rounded-[22px] border border-dashed border-acier bg-white/60">
            {apercu ? (
              ...
              <span className="flex h-full flex-col items-center justify-center gap-2 text-sm text-gris">
                <ImagePlus className="h-8 w-8" strokeWidth={1.2} /> Photo
              </span>
            )}
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <label>
            <span className="libelle">Rôle</span>
            <input className="champ" value={form.role} onChange={maj('role')} required minLength={2} placeholder="Ex. Fondateur, Développeur…" />
          </label>

          <label className="sm:col-span-2">
            <span className="libelle">Courte présentation</span>
          ...
          <label>
            ...
            <span className="mt-1 block text-xs text-gris">Les plus petits nombres s'affichent en premier.</span>
          </label>
